...
-- --------------------------------------------------
-- Date Created: 03/10/2017 22:25:54
-- Generated from EDMX file: D:\Desarrollo\Tio\Repositorio github\SisEquiposBertoncini\SisEquiposBertoncini\Aplicativo\Datos\Model1.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_EquipoEquipo]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Equipos] DROP CONSTRAINT [FK_EquipoEquipo];
GO
...
GO
IF OBJECT_ID(N'[dbo].[FK_EquipoPlanilla_combustible]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Planilla_combustibles] DROP CONSTRAINT [FK_EquipoPlanilla_combustible];
GO
...
GO
IF OBJECT_ID(N'[dbo].[Planilla_combustibles]', 'U') IS NOT NULL
    DROP TABLE [dbo].[Planilla_combustibles];
GO
...
GO
IF OBJECT_ID(N'[dbo].[temp_equipos_excluidos_consulta_mensual]', 'U') IS NOT NULL
    DROP TABLE [dbo].[temp_equipos_excluidos_consulta_mensual];
GO
...
CREATE TABLE [dbo].[Equipos] (
    ...
    [EsTrabajo] bit  NULL,
    [Es_parte_de_id_equipo] int  NULL
);
GO
...
-- Creating table 'Planilla_combustibles'
CREATE TABLE [dbo].[Planilla_combustibles] (
    [id_planilla_combustible] int IDENTITY(1,1) NOT NULL,
    [id_equipo] int  NOT NULL,
    [fecha] datetime  NOT NULL,
    [chofer] nvarchar(max)  NOT NULL,
    [tanque_lleno] bit  NOT NULL,
...
-- Creating table 'temp_equipos_excluidos_consulta_mensual'
CREATE TABLE [dbo].[temp_equipos_excluidos_consulta_mensual] (
    [Id] int IDENTITY(1,1) NOT NULL,
    [Id_equipo] int  NOT NULL
);
GO
...
-- Creating primary key on [id_planilla_combustible] in table 'Planilla_combustibles'
ALTER TABLE [dbo].[Planilla_combustibles]
ADD CONSTRAINT [PK_Planilla_combustibles]
    PRIMARY KEY CLUSTERED ([id_planilla_combustible] ASC);
GO

-- Creating primary key on [Id] in table 'temp_equipos_excluidos_consulta_mensual'
ALTER TABLE [dbo].[temp_equipos_excluidos_consulta_mensual]
ADD CONSTRAINT [PK_temp_equipos_excluidos_consulta_mensual]
    PRIMARY KEY CLUSTERED ([Id] ASC);
GO
...
-- Creating foreign key on [id_equipo] in table 'Planilla_combustibles'
ALTER TABLE [dbo].[Planilla_combustibles]
ADD CONSTRAINT [FK_EquipoPlanilla_combustible]
    FOREIGN KEY ([id_equipo])
    REFERENCES [dbo].[Equipos]
        ([id_equipo])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- Creating non-clustered index for FOREIGN KEY 'FK_EquipoPlanilla_combustible'
CREATE INDEX [IX_FK_EquipoPlanilla_combustible]
ON [dbo].[Planilla_combustibles]
    ([id_equipo]);
GO

-- Creating foreign key on [Es_parte_de_id_equipo] in table 'Equipos'
ALTER TABLE [dbo].[Equipos]
ADD CONSTRAINT [FK_EquipoEquipo]
    FOREIGN KEY ([Es_parte_de_id_equipo])
    REFERENCES [dbo].[Equipos]
        ([id_equipo])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
GO

-- Creating non-clustered index for FOREIGN KEY 'FK_EquipoEquipo'
CREATE INDEX [IX_FK_EquipoEquipo]
ON [dbo].[Equipos]
    ([Es_parte_de_id_equipo]);
GO
